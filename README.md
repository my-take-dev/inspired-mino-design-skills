...
Suite version: 0.8.0
Structural validation: pass (Linux Bash; Windows PowerShell 5.1 over WSL UNC)
...
macOSは現在の検証・release scope外であり、対応済みとは宣言していません。
...
- **Linux**: Bash 4以降を使った構造検証と、case sensitivity、permission、executable bit、symlink等を考慮します。
- **WSL**: Linux filesystem / processを操作するときはLinuxとして扱い、Windows processやWindows側filesystemも操作する場合は境界ごとにplatform Evidenceを分けます。

macOSは現在の検証・release scope外です。利用者側の調整だけで対応済みとは扱わず、将来scopeへ追加する場合はrunner、command、同一requirement / contract / oracleのEvidenceをversioned evaluationへ追加します。
...
WindowsとLinuxの両対応が要件なら、両方の実行結果が揃うまで「検証済み」とは扱いません。利用できないOSは、必要なrunner、command、未実行理由とともに残します。
...
保守時のtraceは「資料テーマ → 判断規則 → 主成果物 → hard gate → case / oracle → evaluation」の順で確認します。0.8.0では、solverへ渡すexact fence payloadを[`cases/0.8.0.md`](.agents/skills/mino-core/evaluations/cases/0.8.0.md)、runner metadataと期待gateを[`oracles/0.8.0.md`](.agents/skills/mino-core/evaluations/oracles/0.8.0.md)へ分離し、実行済み・未実行を[`Evaluation 0.8.0`](.agents/skills/mino-core/evaluations/0.8.0.md)へ記録します。counted runにはmodel / setting、suite / input / output digest、workspace隔離Evidenceが必要です。資料名だけ、schemaの存在だけ、AIの説明だけでは、判断規則が再現されたEvidenceにしません。
...
- repository-local（Windows / Linux共通）: `<target-repository>/.agents/skills/`
...
└── .agents/skills/           # 配布可能なSkill suite
    ├── mino-core/
    │   ├── evaluations/
    │   │   ├── 0.8.0.md             # 現versionのrun結果、未実行事項、残存risk
    │   │   ├── cases/0.8.0.md       # exact fence bodyとして渡すsolver入力
    │   │   ├── oracles/0.8.0.md     # runner metadata、input digest、evaluator-only gate
    │   │   └── fixtures/0.8.0/      # validator parity用のversioned negative fixture
    │   ├── references/platform-compatibility.md
    │   └── scripts/
    │       ├── suite-manifest.txt   # version、owner、Skill一覧
    │       ├── validate-suite.ps1         # Windows
    │       ├── validate-suite.sh          # Linux
    │       ├── test-validator-fixtures.ps1 # Windows validator回帰
    │       └── test-validator-fixtures.sh  # Linux validator回帰
...
7. Windows / Linuxの構造validatorと、対象に必要なruntime testを実行し、未実行事項をevaluationへ残します。
...
| 検証レベル | 確認すること | 0.8.0の状態 |
|---|---|---|
| structural validation | package、front matter、metadata、内部参照、UTF-8 / LF、Linux BashとPowerShell runtime | pass。native Windows checkout / NTFSは含まない |
...
platform Evidenceも同じ「Windows / Linux対応」の一語へまとめません。現在の確認状況は次の四層で記録します。

| Platform evidence | 0.8.0の状態 |
|---|---|
| Linux Bash validator on WSL Linux filesystem | pass。7 Skill / 39 Markdown、Errors 0、Warnings 0。fixture 25 / 25 pass |
| Windows PowerShell 5.1 validator over the same WSL UNC artifact | pass。7 Skill / 39 Markdown、Errors 0、Warnings 0。fixture 25 / 25 pass。PowerShell compatibilityの確認であり、native NTFS Evidenceではない |
| Native Windows checkout / NTFS validator and fixture runner | not executed |
| Application runtime with the same requirement / contract / oracle on Windows and Linux | not executed |

構造validatorのpassだけでbehavioral releaseや対象applicationの正しさを宣言しません。一部caseのpassはその範囲のEvidenceですが、全体の代用にはしません。0.8.0の実行結果、未実行事項、残存riskは[`Evaluation 0.8.0`](.agents/skills/mino-core/evaluations/0.8.0.md)、全release条件は[`Reproducibility benchmark`](.agents/skills/mino-core/references/benchmark.md#release-gate)を参照してください。
...
- 全SkillがWindows / Linux compatibility referenceをroutingしていること。
- Windows用とLinux用のvalidatorがともに同梱されていること。
...
構造validatorの合格は、対象アプリケーションが両OSで正しく動作する証明ではありません。両OS対応を完了とするには、同じrequirementとcontract testをWindows / Linuxそれぞれで実行します。
...
- **OS差を閉じ込めること**: Windows / Linuxで同じcontractを保ち、path、shell、filesystem、processの差だけをimplementationへ隔離します。
