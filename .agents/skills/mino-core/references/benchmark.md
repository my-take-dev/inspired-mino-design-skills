...
再実行可能な評価では、`skills/mino-core/evaluations/cases/0.9.0.md`のsolver input、`skills/mino-core/evaluations/oracles/0.9.0.md`のevaluator-only oracle、`skills/mino-core/evaluations/0.9.0.md`の実行記録を分離する。
...
- Windowsでは`skills/mino-core/scripts/validate-suite.ps1`、Linuxでは`skills/mino-core/scripts/validate-suite.sh`、macOSでは標準system Bashから`skills/mino-core/scripts/validate-suite.sh`を実行してpassする。
- validator契約変更時は`skills/mino-core/scripts/test-validator-fixtures.ps1`と`skills/mino-core/scripts/test-validator-fixtures.sh`で、共通negative fixtureが期待messageとexit 1、capability fixtureがexit 2、positive controlがexit 0になる。
