...
- Windows PowerShell、Linux / macOS Bash validator、fixture parity、native filesystem Evidenceの分離
...
- Windows、Linux、macOSで同じproblem、requirement、contract、canonical decisionを使い、OS差分は`skills/mino-core/references/platform-compatibility.md`に従ってimplementation / environment Evidenceへ分離する。
...
- 複数platform対応がrequiredなのに、いずれかのplatform検証が失敗または未実行のままparityをverifiedとしている
