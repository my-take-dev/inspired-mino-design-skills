...
filesystem、process、shell、toolchain、test実行、またはWindows / Linux / macOS対応がscopeにある場合は、`skills/mino-core/references/platform-compatibility.md`を読む。
...
- 複数platform対応がrequiredで一部を実行できない場合は、必要runnerとcommandを未実行検証へ残す。
...
複数platform対応がrequiredなら、同じrequirement IDとoracleで各platformのtest / build / analysisを実行し、結果を混ぜずに記録する。一つのplatformの成功を他platformのruntime Evidenceとして代用しない。
...
- required platformの検証が失敗、または未実行なのにplatform parityをverifiedとしている
