...
このワークスペースは、`mino-doc/`に整理されたミノ駆動氏の公開ノウハウを、Windows、Linux、macOSで再利用可能なCodex Skillへ変換・保守するために使う。
...
完成したSkillの実行時参照は、インストール先の論理的な`skills/` directory内だけで完結させる。これは、Windows / Linux / macOS、repository-localの`.agents/skills/`、user-wideの`$HOME/.agents/skills/`のいずれでも同じSkillを使えるようにするためのhard ruleである。
...
- shell、filesystem、process、test実行を伴うSkillは`skills/mino-core/references/platform-compatibility.md`をroutingし、Windows、Linux、macOSの差を同じcontractのplatform adapterとして扱う。
- platform固有commandが必要なら、Windows用PowerShellとLinux / macOS用Bashの手順またはscriptを用意し、入力、検査項目、exit statusを一致させる。macOS標準環境を対象にする共用`.sh`は`/bin/bash` 3.2で実行できるsubsetへ制限する。
- Linux Bash構造検証、macOS `/bin/bash`構造検証、Windows PowerShell runtime互換、各native filesystem、対象applicationのplatform runtime parityは別Evidence層として記録し、一つの層のpassを他へ流用しない。
- 一部platformだけで検証した成果物をrequired platform全体で検証済みと宣言しない。実装上の対応範囲とnative runtimeの検証状態を分け、未実行platform、必要runner、commandを明記する。
...
- 各Skillはplatform compatibility referenceを直接routingし、自身の成果物に関係するplatform固有gateと完了条件を持つ。
...
macOSでは標準`/bin/bash`からBash validatorを実行する。
...
- 既存evaluationと、意味を変えた場合に追加した回帰caseを確認する。solver caseとoracleを隔離し、behavioral releaseを主張する場合は最低3 fresh-context runと全Release gateを確認する。複数platform対応がrequiredなら同じoracleを各required platformで実行する。
- validator契約を変えた場合は、Windows / Linux / macOSのapplicableなfixture runnerで同じpositive / negative case、期待message、exit statusを確認する。未実行platformは必要runnerとcommandを残す。
...
- Windows / Linux / macOS、repository-local / user-wideの各配置で内部pathと契約の意味が変わらない。
